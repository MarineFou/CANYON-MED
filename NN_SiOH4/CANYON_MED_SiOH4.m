...
basedir='D:\Documents\Thèse\Docs\science\NN_training\_scripts_\NN\'; % relative or absolute path to CANYON training files
...
cd(basedir)
cd NN_SiOH4
Moy=load('moy_sil.txt');
Ecart=load('std_sil.txt');
...
for i=1:n_list
    b1=load(strcat('weights_biases\poids_sil_b1_',string(i),'.txt'));
    b2=load(strcat('weights_biases\poids_sil_b2_',string(i),'.txt'));
    b3=load(strcat('weights_biases\poids_sil_b3_',string(i),'.txt'));
    IW=load(strcat('weights_biases\poids_sil_IW_',string(i),'.txt'));
    LW1=load(strcat('weights_biases\poids_sil_LW1_',string(i),'.txt'));
    LW2=load(strcat('weights_biases\poids_sil_LW2_',string(i),'.txt'));
