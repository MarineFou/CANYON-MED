function out=CANYON_MED_CT_v4(gtime,lat,lon,pres,temp,psal,doxy)
...
% for example run
% CANYON_MED_CT_v4(datenum(2014,04,09),35,18,500,13.5,38.6,160)
%
% Marine Fourrier, LOV
% 25.08.2020
...
basedir=strcat(fileparts(fileparts(which('CANYON_MED_CT_v4.m'))),'\'); % relative or absolute path to CANYON-MED training files
...
%
mean_nn=mean(CT_outputs_s,2);
std_nn=std(CT_outputs_s,0,2);

for i=1:size(CT_outputs_s,1)
    CT_out(i,1)=mean(CT_outputs_s(i,(CT_outputs_s(i,:)>(mean_nn(i)-std_nn(i))&CT_outputs_s(i,:)<(mean_nn(i)+std_nn(i)))));
end
