function out=CANYON_MED_PO4_v4(gtime,lat,lon,pres,temp,psal,doxy)
...
basedir=strcat(fileparts(fileparts(which('CANYON_MED_PO4_v4.m'))),'\'); % relative or absolute path to CANYON-MED training files
...
%
mean_nn=mean(phos_outputs_s,2);
std_nn=std(phos_outputs_s,0,2);

for i=1:size(phos_outputs_s,1)
    phos_out(i,1)=mean(phos_outputs_s(i,(phos_outputs_s(i,:)>(mean_nn(i)-std_nn(i))&phos_outputs_s(i,:)<(mean_nn(i)+std_nn(i)))));
end
