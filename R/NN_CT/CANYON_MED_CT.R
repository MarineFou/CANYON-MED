CANYON_MED_CT<- function(date,lat,lon,pres,temp,psal,doxy) {
  ...
  Moy <- read.table(paste(basedir,"NN_CT/moy_CT.txt",sep=""))
  Ecart <- read.table(paste(basedir,"NN_CT/std_CT.txt",sep=""))
  ...
  for(i in 1:n_list) {
    ...
    b1=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_b1_',as.character(i),'.txt',sep=""))
    b2=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_b2_',as.character(i),'.txt',sep=""))
    b3=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_b3_',as.character(i),'.txt',sep=""))
    IW=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_IW_',as.character(i),'.txt',sep=""))
    LW1=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_LW1_',as.character(i),'.txt',sep=""))
    LW2=read.table(paste(basedir,'NN_CT/weights_biases/poids_CT_LW2_',as.character(i),'.txt',sep=""))
    b1 <- as.matrix(b1)
    b2 <- as.matrix(b2)
    b3 <- as.matrix(b3)
