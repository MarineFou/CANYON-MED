CANYON_MED_NO3<- function(date,lat,lon,pres,temp,psal,doxy) {
  ...
  Moy <- read.table(paste(basedir,"NN_NO3/moy_nit.txt",sep=""))
  Ecart <- read.table(paste(basedir,"NN_NO3/std_nit.txt",sep=""))
  ...
  for(i in 1:n_list) {
    ...
    b1=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_b1_',as.character(i),'.txt',sep=""))
    b2=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_b2_',as.character(i),'.txt',sep=""))
    b3=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_b3_',as.character(i),'.txt',sep=""))
    IW=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_IW_',as.character(i),'.txt',sep=""))
    LW1=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_LW1_',as.character(i),'.txt',sep=""))
    LW2=read.table(paste(basedir,'NN_NO3/weights_biases/poids_nit_LW2_',as.character(i),'.txt',sep=""))
    b1 <- as.matrix(b1)
    b2 <- as.matrix(b2)
    b3 <- as.matrix(b3)
