CANYON_MED_CT<- function(date,lat,lon,pres,temp,psal,doxy) {
  ...
  #
  # check value:  2314.1664 umol kg-1
  ...
  Moy <- read.table(paste(basedir,"CANYON-MED_weights/moy_CT.txt",sep=""))
  Ecart <- read.table(paste(basedir,"CANYON-MED_weights/std_CT.txt",sep=""))
  ...
  CT_outputs_s=rep(0,n_list)
  
  rx <-dim(data_N)[1]
  for(i in 1:n_list) {
    
    b1=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_b1_',as.character(i),'.txt',sep=""))
    b2=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_b2_',as.character(i),'.txt',sep=""))
    b3=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_b3_',as.character(i),'.txt',sep=""))
    IW=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_IW_',as.character(i),'.txt',sep=""))
    LW1=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_LW1_',as.character(i),'.txt',sep=""))
    LW2=read.table(paste(basedir,'CANYON-MED_weights/poids_CT_LW2_',as.character(i),'.txt',sep=""))
    b1 <- as.matrix(b1)
    b2 <- as.matrix(b2)
    b3 <- as.matrix(b3)
    ...
    CT_outputs=1.5*y*Ecart[1,ne+1]+Moy[1,ne+1]
    
    CT_outputs_s[i]=CT_outputs
  }
  
  CT_out=mean(CT_outputs_s);
